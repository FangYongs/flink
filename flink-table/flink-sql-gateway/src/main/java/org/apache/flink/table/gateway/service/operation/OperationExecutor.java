...
public class OperationExecutor {
    ...
    private ResultFetcher executeOperation(
        ...
        if (op instanceof SetOperation) {
            ...
            return callQueryOperation(tableEnv, handle, ((QueryOperation) op));
    ...
    private ResultFetcher callQueryOperation(
            TableEnvironmentInternal tableEnv, OperationHandle handle, QueryOperation queryOp) {
        QueryOperation operation = queryOp;
        if (sessionContext.getTableSnapshotProvider().isPresent()) {
            TableSnapshotProvider provider = sessionContext.getTableSnapshotProvider().get();

            TableSnapshotVisitorFactory visitorFactory =
                    TableFactoryUtil.buildVisitorFactory(
                            sessionContext.getUserClassloader());
            Set<ObjectIdentifier> tables = queryOp.accept(visitorFactory.createTableVisitor());
            Map<ObjectIdentifier, Long> tableSnapshots =
                    provider.calcTableSnapshot(
                            new TableSnapshotProvider.TableSnapshotContext() {
                                @Override
                                public Set<ObjectIdentifier> tables() {
                                    return tables;
                                }

                                @Override
                                public Optional<CatalogDescriptor> catalog(
                                        String catalogName) {
                                    return sessionContext
                                            .getSessionState()
                                            .catalogStoreHolder
                                ...
                                @Override
                                public ReadableConfig jobConfiguration() {
                                    return tableEnv.getConfig().getConfiguration();
                                }
                            });
            operation = queryOp.accept(visitorFactory.createSnapshotVisitor(tableSnapshots));
        }
        TableResultInternal result = tableEnv.executeInternal(operation);
